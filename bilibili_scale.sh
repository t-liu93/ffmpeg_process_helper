...
inotifywait -m $WD -e create -e close_write -e moved_to -e modify | 
    while read dir action file; do
        ...
        FPS=$(expr $HIGH / $LOW)
        GOP=$(expr $FPS \* 5)
        $FFMPEG/ffmpeg -y -i $WD/$file -i $WD/../bilibiliwatermark.png  -c:v libx264 -b:v 5850k -maxrate 11M -bufsize 22M -preset slow -profile:v high -level 4.2 -filter_complex "overlay=0:0,format=yuv420p,scale=w=1920:h=1080:sws_flags=lanczos" -pass 1 -passlogfile /home/tianyu/ffmpeg/ffmpeglog -an -f mp4 /dev/null
